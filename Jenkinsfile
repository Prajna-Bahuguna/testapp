            ...
            steps {
                script {
                    ...
                    sshagent(['HELM_SERVER_SSH_CREDENTIALS']) {
                        ...
                        sh "ssh helm-server-user@helm-server-ip 'helm upgrade ${HELM_RELEASE_NAME} ${HELM_CHART_PATH} --install --set image.repository=prajnab9/myapp --set image.tag=${BUILD_NUMBER}'"
                    }
                }
            }
        }
